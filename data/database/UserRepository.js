...
class UserRepository {
  ...
  updateUserProfile(req, res) {
    const { userId } = req.session;
    const { Username, Password, Email, Role, CraftSkill, CraftInterest, ProfilePicture } = req.body.user;
    ...
    db.query(
      ...
      (error, results) => { 
        ...
        if (CraftInterest) {
          updateFields.push(`CraftInterest = JSON_SET(craftinterest, "$.key", ?)`);
          updateValues.push(CraftInterest.key);
        }
